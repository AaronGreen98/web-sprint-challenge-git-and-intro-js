...
function addArtist(arrayvalue, namevalue, yearsvalue, genrevalue, nationalityvalue, biovalue) {
  arrayvalue.push({"name": namevalue, "years": yearsvalue, "genre": genrevalue, "nationality": nationalityvalue, "bio": biovalue}) 
  return (arrayvalue)
} 
console.log('task 6', addArtist(artists, "Aaron", "1998-2022", "Full Stack Development", "White", "I am attending Bloomtech to gain coding knowledge and to make a career out of it."));
...
function lotsOfArt(arrayvalue){
  let filteredarray = [];
  for (let i = 0; i < arrayvalue.length; i ++){
    if (arrayvalue[i].paintings > 100){
      filteredarray.push(arrayvalue[i].name);
    }
  }
...
function artistInfo (arrayvalue, namevalue){
for (let i = 0; i < arrayvalue.length; i ++){
  if (arrayvalue[i].name === namevalue){
    return (arrayvalue[i].bio)
  }
}
...
function artistByCountry(arrayvalue, nationalityvalue){
  let filteredarray = [];
  for (let i = 0; i < arrayvalue.length; i ++){
    if (arrayvalue[i].nationality === nationalityvalue){
      filteredarray.push(arrayvalue[i].name);
    }
  }
  return (filteredarray)
